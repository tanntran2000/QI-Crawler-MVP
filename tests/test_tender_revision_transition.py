...
def test_newer_acceptance_advances_and_preserves_previous(transition_service) -> None:
    ...
    assert result.outcome is RevisionTransitionOutcome.ACCEPTED_PENDING
...
def test_adjacent_diff_marks_removed_and_rejects_non_adjacent(transition_service) -> None:
    diff = transition_service.compare_adjacent_revisions(
        ...
        new_source_observation_complete=True, completeness_evidence="full source manifest",
    )
...
def test_newer_acceptance_is_pending_and_latest_unchanged(transition_service) -> None:
    transition_service.accept_revision("case-revision", 1, actor="Team Bid", reason="first", evidence="evidence")
    result = transition_service.accept_revision(
        "case-revision", 2, actor="Team Bid", reason="revision", evidence="evidence"
    )
    assert result.outcome.value == "ACCEPTED_PENDING"
    assert result.latest.identity.revision == "00"
    pending = transition_service.persistence.latest_pending("case-revision")
    assert pending is not None
    assert pending.revision == "01"


def test_pending_transition_survives_restart(transition_service) -> None:
    transition_service.accept_revision("case-revision", 1, actor="Team Bid", reason="first", evidence="evidence")
    transition_service.accept_revision("case-revision", 2, actor="Team Bid", reason="revision", evidence="evidence")
    reopened = TenderRevisionTransitionService(transition_service.persistence)
    assert reopened.persistence.latest_pending("case-revision").revision == "01"
    assert reopened.operational_latest("case-revision").revision == "00"


def test_activation_without_exact_comparison_holds(transition_service) -> None:
    transition_service.accept_revision("case-revision", 1, actor="Team Bid", reason="first", evidence="evidence")
    transition_service.accept_revision("case-revision", 2, actor="Team Bid", reason="revision", evidence="evidence")
    result = transition_service.activate_revision(
        "case-revision", 2, actor="Team Bid", reason="activate", evidence="comparison"
    )
    assert result.outcome.value == "HOLD"
    assert result.latest.identity.revision == "00"


def test_explicit_activation_after_comparison_advances(transition_service) -> None:
    transition_service.accept_revision("case-revision", 1, actor="Team Bid", reason="first", evidence="evidence")
    transition_service.accept_revision("case-revision", 2, actor="Team Bid", reason="revision", evidence="evidence")
    diff = transition_service.compare_adjacent_revisions(
        "case-revision", 1, 2, previous_snapshot={"C3": "old"}, latest_snapshot={"C3": "new"}
    )
    assert diff["C3"].state.value == "CHANGED"
    result = transition_service.activate_revision(
        "case-revision", 2, actor="Team Bid", reason="activate", evidence="comparison"
    )
    assert result.outcome.value == "ADVANCED"
    assert result.latest.identity.revision == "01"
    assert transition_service.persistence.operational_latest("case-revision").revision == "01"


def test_incomplete_previous_only_is_unknown_relation(transition_service) -> None:
    diff = transition_service.compare_adjacent_revisions(
        "case-revision", 1, 2, previous_snapshot={"C3": "old"}, latest_snapshot={}
    )
    assert diff["C3"].state.value == "UNKNOWN_RELATION"


def test_complete_previous_only_can_be_removed(transition_service) -> None:
    diff = transition_service.compare_adjacent_revisions(
        "case-revision", 1, 2, previous_snapshot={"C3": "old"}, latest_snapshot={},
        new_source_observation_complete=True, completeness_evidence="full source manifest",
    )
    assert diff["C3"].state.value == "REMOVED_FROM_NEW_REVISION"


def test_conflicting_pending_transition_fails_closed(transition_service) -> None:
    transition_service.accept_revision("case-revision", 1, actor="Team Bid", reason="first", evidence="evidence")
    transition_service.accept_revision("case-revision", 2, actor="Team Bid", reason="revision", evidence="evidence")
    result = transition_service.accept_revision(
        "case-revision", 3, actor="Team Bid", reason="another revision", evidence="evidence"
    )
    assert result.outcome is RevisionTransitionOutcome.HOLD
    assert result.latest.identity.revision == "00"
    assert result.pending.revision == "01"


def test_activation_appends_immutable_event_with_exact_pair(transition_service) -> None:
    transition_service.accept_revision("case-revision", 1, actor="Team Bid", reason="first", evidence="evidence")
    transition_service.accept_revision("case-revision", 2, actor="Team Bid", reason="revision", evidence="evidence")
    transition_service.compare_adjacent_revisions(
        "case-revision", 1, 2, previous_snapshot={"C3": "old"}, latest_snapshot={"C3": "new"}
    )
    transition_service.activate_revision(
        "case-revision", 2, actor="Team Bid", reason="activate", evidence="comparison"
    )
    events = transition_service.persistence.events_for_case("case-revision")
    assert [event.decision for event in events] == [
        "ACCEPTED", "ACCEPTED_PENDING", "COMPARISON", "TRANSITION_ACTIVATED"
    ]
    assert events[-1].from_release_id == 1
    assert events[-1].comparison_schema_version == "adjacent-v1"
