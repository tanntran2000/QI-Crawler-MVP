...
```text
HANDOFF_ID = WP-BID-RADAR-HARDENING-01 / MICRO-A6-C1-R1
CANONICAL_CHECKOUT = D:\QI Technology\QI Crawler\egp-crawler-python
PARENT_WP = WP-BID-RADAR-HARDENING-01
CURRENT_PHASE = MICRO-A6-C1-R1_SOURCE_SESSION_AND_GENERIC_FIND_CORRECTION / IMPLEMENTATION
ACTIVE_PRODUCT_WP = WP-BID-RADAR-HARDENING-01
ACTIVE_ENGINEERING_WP = WP-BID-RADAR-HARDENING-01 / MICRO-A6-C1-R1
PARENT_STATE = AUTHORIZED / MICRO-A6-C1-R1_IMPLEMENTED_PENDING_PLANNER_AND_HUMAN_REVIEW
PRODUCT_CHANGE = NO
VERSION_IMPACT = NONE
...
ACTIVE_MICRO_WP = MICRO-A6-C1-R1
...
NEXT = STOP_FOR_PLANNER_AND_HUMAN_A6_C1_R1_REVIEW
EXACTLY_ONE_NEXT_ACTION = STOP_FOR_PLANNER_AND_HUMAN_A6_C1_R1_REVIEW
...
Micro-A6-C1-R1 keeps the explicit session-only pending/active source model, removes the Bid Radar active-package layer, and makes the selected exact opportunity the direct Review and controlled workspace-handoff target. Generic Find is a case-insensitive, accent-sensitive literal substring search over eligible source-origin business fields, with Find-hit counts separate from final filter matches. The source-switch freeze was not reproduced under controlled timing and modal-responsiveness checks; no freeze workaround was added. Micro-A5 remains Human-accepted/closed, A6-C2 location and A7 export remain inactive, and this bounded correction is pending Planner and Human A0 review; no release, migration-file, or product-authority scope was expanded.
