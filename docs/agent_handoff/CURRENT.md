...
MICRO_STATE = DISCOVERY_PENDING_IMPLEMENTATION_AUTHORIZATION
...
ARCHITECTURE_LAYER_CONTRACT = DOMAIN + APPLICATION + PERSISTENCE DISCOVERY ONLY

02B_1 = MERGED
02B_2 = MERGED
02B_3 = PRE / DISCOVERY ONLY
...
SCOPE_BOUNDARIES = 02B-3 exact-revision Human Review persistence discovery only; no production implementation yet; no confirmed export; no GUI; no API; no Tender Package creation; no scoring/ranking/GO-HOLD-NO-GO; no review inheritance across revisions

OPEN_BLOCKERS = Exact persistence architecture for source-neutral Human Review must be determined without breaking legacy KHMT CandidateReviewService
EXACTLY_ONE_NEXT_ACTION = BUILDER DISCOVER 02B-3 REVIEW PERSISTENCE BOUNDARY
