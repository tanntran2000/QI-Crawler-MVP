...
```text
HANDOFF_ID = WP-BID-RADAR-HARDENING-01 / MICRO-A6
CANONICAL_CHECKOUT = D:\QI Technology\QI Crawler\egp-crawler-python
PARENT_WP = WP-BID-RADAR-HARDENING-01
CURRENT_PHASE = MICRO-A6_ACTIVE_TENDER_CONTEXT_AND_REVIEW_HANDOFF / IMPLEMENTATION
ACTIVE_PRODUCT_WP = WP-BID-RADAR-HARDENING-01
ACTIVE_ENGINEERING_WP = WP-BID-RADAR-HARDENING-01 / MICRO-A6
PARENT_STATE = AUTHORIZED / MICRO-A6_IMPLEMENTED_PENDING_PLANNER_AND_HUMAN_REVIEW
PRODUCT_CHANGE = NO
VERSION_IMPACT = NONE
...
A6_ACTIVE_TENDER_CONTEXT = IMPLEMENTED_PENDING_PLANNER_AND_HUMAN_REVIEW
...
A6 = IMPLEMENTED_PENDING_PLANNER_AND_HUMAN_REVIEW
...
NEXT = STOP_FOR_PLANNER_AND_HUMAN_A6_REVIEW
EXACTLY_ONE_NEXT_ACTION = STOP_FOR_PLANNER_AND_HUMAN_A6_REVIEW
...
Micro-A6 adds a session-only Active Working Package with exact source type, raw/base/revision identity, source hash, and observation key. Row selection remains preview-only; activation and switching require explicit Human confirmation, while review and Tender Bid handoff require selected identity to match the active exact revision plus existing persisted review authority. Source changes require confirmation and clear session context; same-source refilters preserve it. Micro-A5 remains Human-accepted/closed, and this bounded implementation is pending Planner and Human A0 review; no release, migration-file, or product-authority scope was expanded.
