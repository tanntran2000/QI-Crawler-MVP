...
schema version. `ProcurementPlan` preserves `plan_base_id` and
`plan_revision`. Each `PlanPackage` preserves its source row, raw and
normalized values, pricing, investor, project, method, schedule, and
provenance.

Province/city is the sole MI MVP geography. Its status is `CONFIRMED`,
`INFERRED`, or `NEEDS_REVIEW`; unresolved locations are never guessed.
