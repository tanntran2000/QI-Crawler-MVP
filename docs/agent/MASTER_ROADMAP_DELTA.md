...
```text
DELTA_ID = RD-0012
STATE = STAGE02C_IMPLEMENTED_PENDING_FOCUSED_INDEPENDENT_REAUDIT
...
NEW_HUMAN_FINDING = OA05_OA07_POST_MUTATION_PROJECTION_STALENESS
NEW_FINDING_CORRECTION = IMPLEMENTED_PENDING_FOCUSED_REAUDIT
...
NEXT = PUSH_PR121_THEN_FRESH_EXACT_HEAD_HOSTED_CI_AND_FOCUSED_INDEPENDENT_UI_REFRESH_REAUDIT
```

These corrections remain pending independent audit and later Human GUI
acceptance on a newly built candidate. They do not close the OA findings or
change the historical acceptance status of the existing candidate.
