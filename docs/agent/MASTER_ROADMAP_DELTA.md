...
```text
DELTA_ID = RD-0011
STATE = HUMAN_AUTHORIZED_READINESS_PARENT_IN_PROGRESS
...
ACTIVE_PARENT = WP-REL-V010-CANDIDATE-READINESS-01
...
SEQUENCE =
  ...
  → CAPABILITY_ACCEPTANCE_66_OF_66
  → HUMAN_PROMOTION_DECISION

BOUNDARIES =
  NO_FINAL_CANDIDATE_BUILD;
  NO_REAL_TEAM_BID_DATA_CLONE_OR_MIGRATION;
...
READINESS_IMPLEMENTATION = AUDIT_CORRECTIONS_LOCALLY_VERIFIED_PENDING_INDEPENDENT_REAUDIT
...
FROZEN_MIGRATION_EXECUTION_BINDING = CLEAN_EXACT_HEAD_PLUS_GIT_BLOB_MATCH_LOCALLY_VERIFIED
PORTABLE_ARTIFACT_IDENTITY = QI_CRAWLER_PORTABLE_ARTIFACT_V1_NO_INSTALLER_HASH
DIRECT_CANDIDATE_STARTUP = FAIL_CLOSED_UNTIL_VALID_ACCEPTANCE
POST_ACCEPTANCE_RUNTIME = CANDIDATE_ROOT_BOUND_WITH_MUTABLE_OPERATIONAL_DB_SUPPORTED
...
This delta captures the active Human release direction. It does not promote
v0.10.0 to a built, accepted, installed or released state.
